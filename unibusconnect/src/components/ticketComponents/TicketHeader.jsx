...
const TicketHeader = ({ date, from, to }) => {
  ...
  const fromContent = splitFromAddress[1]
    ? splitFromAddress[1]
    : splitFromAddress[0];
  const toContent = splitToAddress[1] ? splitToAddress[1] : splitToAddress[0];
  return (
    <div className="bg-white border border-warning rounded-1 shadow-sm p-3 mb-3">
      ...
      <div className="row mx-0">
        <div className="col-6 p-0">
          <small className="text-muted mb-1 f-10 pr-1">DATE OF JOURNEY</small>
          <p className="small mb-0 l-hght-14"> {date?.split("T")[0]}</p>
        </div>
        <div className="col-6 p-0">
          <small className="text-muted mb-1 f-10 pr-1">YOU RATED</small>
